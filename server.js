...
var http = require("http");//기본 웹서버 모듈 
var mysql = require("mysql");//외부 모듈 가져오기 
var querystring = require("querystring");//post파라미터 처리 모듈 
var fs = require("fs"); //파일시스템 내장 모듈 
var url = require("url"); //resolution, parsing 

//node.js의 모듈중jsp,php,asp와 같은 서버에서만 해석및 실행되는
//스크립트기술이 지원된다 
//이를 지원하는 모듈이 ejs이다!! 
var ejs = require("ejs");
...
var server = http.createServer(function(request, response){
    ...
    //단순 문자열 비교를 하려고 한다 파라미터가 포함되어 있을 경우 
    //문자열이 틀리다고 생각해버린다 
    //URL 전문적으로 다루는 모듈 도입 
    const path = url.parse(request.url).pathname;

    switch(path){
        case "/hero/join":joinform(response);break; //등록폼요청 
...
function insert(request,response){
    //넘겨받은  request객체로 부터 파라미터 뽑아내기 !!
    //입력 양식으로 전송될 예정이므로 post로 전송될 것이다 
    //node.js에서 post로 전송될때는 querystring모듈을 사용해보자 
    //post방식은 request객체의 on이벤트 핸들러로 데이터가 전송된 시점에 처리해야한다 

    var content = ""; //이벤트가 여러번 발생할 수도 있기에 변수에 데이터 모아놓자 
...
function selectAll(request, response){
   ...
   //두번째인수인 result에 레코드셋이 담겨진다 
   client.query(sql, function(err, result, fields){
    if(err){
        console.log(err);
    }else{
        console.log(result);
        //레코드셋 집합의 정보가 list.ejs에 반영되도록 해야한다 
        response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
...
function select(request,response){
    ...
    client.query(sql,[json.hero_id],function(err,result,fields){
        ...
        fs.readFile("detail.ejs","utf8", function(err,data){
            response.end(ejs.render(data,{
                record:result[0]//result에는 한건이 들어있지만 배열이므로 
            }));
        });
...
function update(request,response){
   ...
   request.on("data",function(data){
        ...
        console.log("name:",json.gender);
        console.log("name:",json.age);
